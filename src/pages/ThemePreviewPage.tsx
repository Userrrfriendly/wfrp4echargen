...
function ColorSwatch({ label, value }: { label: string; value: string }) {
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0.75, minWidth: 96 }}>
      <Box
        sx={{
          width: 64,
          height: 64,
      ...
      <Typography variant="caption" color="text.secondary" align="center" sx={{ fontFamily: 'monospace' }}>
        {value}
      </Typography>
    </Box>
  );
...
const TYPOGRAPHY_VARIANTS = [
  ...
  { variant: 'subtitle1', label: 'Subtitle 1 — supporting text under a heading' },
  { variant: 'subtitle2', label: 'Subtitle 2 — smaller supporting text' },
  { variant: 'body1', label: 'Body 1 — this is the default body text style used throughout the application for all readable content.' },
  { variant: 'body2', label: 'Body 2 — slightly smaller body text (avoid in favour of body1).' },
...
export default function ThemePreviewPage() {
  ...
  return (
    <Box sx={{ maxWidth: 960 }}>
      ...
      <Typography component="div" variant="body1" color="text.secondary" sx={{ mb: 2 }}>
        Current mode:{' '}
        <Chip label={p.mode} size="small" variant="outlined" sx={{ fontFamily: 'monospace' }} />
      </Typography>
        ...
        <Section title="Text">
          <SwatchRow
            colors={[
              ...
              { label: 'text.icon', value: (p.text as { icon?: string }).icon ?? 'rgba(255,255,255,0.5)' },
            ]}
          />
        </Section>
        ...
        <Section title="Action — Colors">
          <SwatchRow
            colors={[
              ...
              { label: 'action.disabledBackground', value: p.action.disabledBackground },
      ...
      <TabPanel value={tab} index={1}>
        {TYPOGRAPHY_VARIANTS.map(({ variant, label }) => {
          ...
          const fontFamily =
            typeof style === 'object' && style !== null && 'fontFamily' in style
              ? String((style as { fontFamily?: string }).fontFamily ?? theme.typography.fontFamily)
          ...
          const fontSize =
            typeof style === 'object' && style !== null && 'fontSize' in style
              ...
              : '';
          const fontWeight =
            typeof style === 'object' && style !== null && 'fontWeight' in style

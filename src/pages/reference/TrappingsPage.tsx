...
function TrappingStats({
  type,
  object,
}: {
  type: number;
  object: ReturnType<typeof useTrappings>['data'] extends
    | (infer T)[]
    | undefined
  ...
  if (
    type === 0 &&
    object.melee.group >= 0 &&
    (object.melee.dmg > 0 || object.melee.dmgSbMult > 0)
  ) {
    const dmgStr =
      [
    ...
    return (
      <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mt: 1 }}>
        ...
        <Typography variant="body1" color="text.secondary">
          Hands:{' '}
          {object.melee.hands === 2
            ? 'Two-handed'
            : object.melee.hands === 1
              ? 'One-handed'
              : '—'}
        </Typography>
      </Box>
    ...
    return (
      <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mt: 1 }}>
        ...
        <Typography variant="body1" color="text.secondary">
          ...
          {object.ranged.rngSbMult > 0
            ? ` (+${object.ranged.rngSbMult}×SB)`
            : ''}
        </Typography>
        <Typography variant="body1" color="text.secondary">
          Group:{' '}
          {RANGED_GROUPS[object.ranged.group] ?? `G${object.ranged.group}`}
        </Typography>
      </Box>
    );
  }
  if (type === 3 && object.armour.points > 0) {
    const locs = object.armour.location
      .map((l) => ARMOUR_LOCATIONS[l] ?? `Loc${l}`)
      .join(', ');
    return (
      <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mt: 1 }}>
        <Typography variant="body1" color="text.secondary">
          AP: {object.armour.points}
        </Typography>
        {locs && (
          <Typography variant="body1" color="text.secondary">
            Covers: {locs}
  ...
  if (error) {
    return (
      <Typography color="error">
        Failed to load trappings: {(error as Error).message}
      </Typography>
    );
  }
            ...
            <ListItemButton>
              <Box sx={{ py: 0.25, width: '100%' }}>
                ...
                >
                  <Chip
                    label={
                      TRAPPING_TYPES[item.object.type] ??
